...
for i in range(len(beta_arr)):
    if interp_index_high[i] == interp_index_low[i]:
        time, dmdt = np.genfromtxt(dmdtdir + '{0:.3e}'.format(beta_arr[i])[:5]+ '.dat', skip_header=1, unpack=True)
        np.savetxt(savedir + '{0:.3e}'.format(beta_arr[i])[:5]+ '.dat',
        ...
        np.savetxt(savedir + '{0:.3e}'.format(beta_arr[i])[:5]+ '.dat', np.transpose([np.concatenate([timeinterpolated[0], timeinterpolated[1]]),
