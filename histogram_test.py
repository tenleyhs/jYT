...
if (set(['bhbound','selfbound','angmom']) & set(args.vars + args.excludevars)):
	...
	def func(x):
		return (pf.current_time.v - edata[2]) * 2. * np.pi / edata[1] - x + edata[5] * np.sin(x)

	E = optimize.fsolve(func, 0.01)[0]
	pericenter_radius = (1./edata[0])*edata[3]*np.power(edata[6]/edata[4], 1./3.)
